  ...
  <div class="">
    ...
    <div class="">
      <div class="row container position-absolute bottom-0 start-50 translate-middle-x">
        ...
        <div class="col-4 d-grid">
          <button @click="next(-1)" class="btn btn-lg" :class="{'btn-primary': isActive[-1], 'btn-outline-primary': !isActive[-1]}">NEM</button>
        </div>
        <div class="col-4 d-grid gap-1">
          <button @click="next(0)" class="btn btn-lg text-muted" :class="{'btn-dark': isActive[0], 'btn-outline-secondary': !isActive[0]}" ><small>Neutral</small></button>
        </div>
        <div class="row pt-2">
          <div class="col-6 d-grid d-flex justify-content-start">
            <button v-if="notFirst" @click="back(null)" class="btn btn-link text-muted">&lt; Back</button>
          </div>
          <div class="col-6 d-grid d-flex justify-content-end">
            <button @click="next(null)" class="btn btn-link text-muted" :class="{'btn-dark': isActive[null], '': !isActive[null]}">Skip ></button>
          </div>
        </div>
      </div>
    </div>
